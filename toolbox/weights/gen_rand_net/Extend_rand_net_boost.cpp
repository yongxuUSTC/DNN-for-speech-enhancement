...
void Initial(int argc, char **argv)
{
	...
	else
	{
		...
		for(i =1; i< numlayers; i++)
		{
			...
			int size = ori_layersizes[i-1]*ori_layersizes[i];
			...
			{
				weights[i][m*layersizes[i]+n]=tmp_weights[rand()%size];
			}
            ...
            ////
			for(m=0; m<ori_layersizes[i];m++)
			{
				bias[i][m]=tmp_bias[m];
			}
            for(m=ori_layersizes[i];m<layersizes[i];m++)
		    {
			    bias[i][m]=tmp_bias[rand()%ori_layersizes[i]];
		    }
		}
	    fclose(fp_in);
...
int main(int argc, char *argv[])
{
	...
	if (argc==1)
	{
		cout<<"numlayers ori_layersizes add_layersizes in_pfile out_pfile"<<endl;
	}
	Initial(argc, argv);
	Writeweights();
////	///////test
